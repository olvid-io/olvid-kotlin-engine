...
import io.olvid.engine.datatypes.containers.DialogType.Companion.createSyncItemToApplyDialog
...
class SynchronizationProtocol(
    ...
    override val protocolId: Int = ConcreteProtocol.SYNCHRONIZATION_PROTOCOL_ID
    ...
    override fun getMessageClass(protocolMessageId: Int): Class<*>? {
        when (protocolMessageId) {
            INITIATE_SINGLE_ITEM_SYNC_MESSAGE_ID -> return InitiateSingleItemSyncMessage::class.java
            SINGLE_ITEM_SYNC_MESSAGE_ID -> return SingleItemSyncMessage::class.java
            else -> return null
        }
    }

    class InitiateSingleItemSyncMessage : ConcreteProtocolMessage {
        @JvmField val obvSyncAtom: ObvSyncAtom

        constructor(coreProtocolMessage: CoreProtocolMessage?, obvSyncAtom: ObvSyncAtom) : super(
            coreProtocolMessage!!
        ) {
        ...
        override val protocolMessageId: Int = INITIATE_SINGLE_ITEM_SYNC_MESSAGE_ID
    ...
    class SingleItemSyncMessage : ConcreteProtocolMessage {
        @JvmField val obvSyncAtom: ObvSyncAtom
        ...
        override val protocolMessageId: Int = SINGLE_ITEM_SYNC_MESSAGE_ID
    ...
    override fun getPossibleStepClasses(stateId: Int): Array<Class<*>> {
        when (stateId) {
            ConcreteProtocol.INITIAL_STATE_ID -> return arrayOf<Class<*>>(
                SendSingleItemSyncMessageStep::class.java,
                ProcessSingleItemSyncMessageStep::class.java /*, UpdateStateAndSendSyncMessageStep.class*/
            )
    ...
    class SendSingleItemSyncMessageStep(
        @field:Suppress("unused") internal val startState: InitialProtocolState?,
        internal val receivedMessage: InitiateSingleItemSyncMessage,
...
//                }
    ...
    class ProcessSingleItemSyncMessageStep(
        @field:Suppress("unused") internal val startState: InitialProtocolState?,
        internal val receivedMessage: SingleItemSyncMessage,
        protocol: SynchronizationProtocol?
    ) : ProtocolStep(
            ...
            if (receivedMessage.obvSyncAtom.isAppSyncItem) {
                ...
                val coreProtocolMessage = buildCoreProtocolMessage(
                    createUserInterfaceChannelInfo(
                        ownedIdentity,
                        createSyncItemToApplyDialog(receivedMessage.obvSyncAtom),
                        UUID.randomUUID()
                    )
                )
                ...
                protocolManagerSession.identityDelegate!!.processSyncItem(
                    protocolManagerSession.session,
                    ownedIdentity,
                    receivedMessage.obvSyncAtom
                )
...
//            }
    ...
    companion object {
        ...
        // endregion
        // region Messages
        const val INITIATE_SINGLE_ITEM_SYNC_MESSAGE_ID: Int = 0
        const val SINGLE_ITEM_SYNC_MESSAGE_ID: Int = 1
    }
}
