...
class DownloadGroupV2PhotoProtocol(
    ...
    override val protocolId: Int = ConcreteProtocol.DOWNLOAD_GROUPS_V2_PHOTO_PROTOCOL_ID
    ...
    class DownloadingPhotoState : ConcreteProtocolState {
        ...
        internal val serverPhotoInfo: ServerPhotoInfo

        constructor(groupIdentifier: GroupV2.Identifier, serverPhotoInfo: ServerPhotoInfo) : super(
        ...
        constructor(encodedState: Encoded) : super(DOWNLOADING_PHOTO_STATE_ID) {
            ...
            this.serverPhotoInfo = ServerPhotoInfo.of(list[1])
        }
    ...
    class PhotoDownloadedState : ConcreteProtocolState(PHOTO_DOWNLOADED_STATE_ID) {
        override fun encode(): Encoded {
    ...
    class QueryServerStep(
        ...
        protocol: DownloadGroupV2PhotoProtocol?
    ) : ProtocolStep(
            ...
            if (receivedMessage.groupIdentifier.category == GroupV2.Identifier.CATEGORY_KEYCLOAK) {
                val coreProtocolMessage = buildCoreProtocolMessage(
                    createServerQueryChannelInfo(
                        ownedIdentity,
                        GetKeycloakDataQuery(
                            receivedMessage.groupIdentifier.serverUrl,
                            receivedMessage.serverPhotoInfo.serverPhotoLabel
                        )
                    )
                )
                val messageToSend: ChannelMessageToSend? =
                    DownloadGroupV2PhotoProtocol.ServerGetPhotoMessage(coreProtocolMessage)
                ...
                return DownloadingPhotoState(
                    ...
                    receivedMessage.serverPhotoInfo
                )
            } else {
                val coreProtocolMessage = buildCoreProtocolMessage(
                    createServerQueryChannelInfo(
                        ownedIdentity,
                        GetUserDataQuery(
                            receivedMessage.serverPhotoInfo.serverPhotoIdentity!!,
                            receivedMessage.serverPhotoInfo.serverPhotoLabel,
                            true
                        )
                    )
    ...
    class ProcessPhotoStep(
        ...
        override fun executeStep(): ConcreteProtocolState {
            ...
            val decryptedPhoto = Suite.getAuthEnc(startState.serverPhotoInfo.serverPhotoKey)!!
                .decrypt(startState.serverPhotoInfo.serverPhotoKey, receivedMessage.encryptedPhoto)

            protocolManagerSession.identityDelegate!!.setGroupV2DownloadedPhoto(
                protocolManagerSession.session,
                ownedIdentity,
                startState.groupIdentifier,
                startState.serverPhotoInfo,
                decryptedPhoto
            )
