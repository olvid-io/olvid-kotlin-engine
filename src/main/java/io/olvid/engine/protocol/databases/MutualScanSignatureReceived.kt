...
class MutualScanSignatureReceived : ObvDatabase {
...
//    }
    ...
    override fun delete() {
        protocolManagerSession.session.prepareStatement(
            "MutualScanSignatureReceived.delete",
    ...
    companion object {
        const val TABLE_NAME: String = "mutual_scan_signature_received"
        ...
        fun create(
            ...
            ownedIdentity: Identity?,
            signature: ByteArray?
        ): MutualScanSignatureReceived? {
            if (ownedIdentity == null || signature == null) {
                return null
            }
            try {
                val mutualScanSignatureReceived =
                    MutualScanSignatureReceived(protocolManagerSession, ownedIdentity, signature)
                mutualScanSignatureReceived.insert()
        ...
        fun upgradeTable(session: Session, oldVersion: Int, newVersion: Int) {
            ...
            if (oldVersion < 23 && newVersion >= 23) {
                ...
                session.createStatement().use { statement ->
                    statement.execute(
                        "CREATE TABLE IF NOT EXISTS mutual_scan_signature_received (" +
                                " owned_identity BLOB NOT NULL, " +
                                " signature BLOB NOT NULL, " +
        ...
        fun exists(
            ...
            protocolManagerSession.session.prepareStatement(
                "MutualScanSignatureReceived.exists",
