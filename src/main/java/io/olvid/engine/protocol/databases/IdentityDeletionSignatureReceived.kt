...
class IdentityDeletionSignatureReceived : ObvDatabase {
...
//    }
    ...
    override fun delete() {
        protocolManagerSession.session.prepareStatement(
            "IdentityDeletionSignatureReceived.delete",
    ...
    companion object {
        const val TABLE_NAME: String = "identity_deletion_signature_received"
        ...
        fun create(
            ...
            ownedIdentity: Identity?,
            signature: ByteArray?
        ): IdentityDeletionSignatureReceived? {
            if (ownedIdentity == null || signature == null) {
                return null
            }
            try {
                val identityDeletionSignatureReceived = IdentityDeletionSignatureReceived(
        ...
        fun upgradeTable(session: Session, oldVersion: Int, newVersion: Int) {
            ...
            if (oldVersion < 33 && newVersion >= 33) {
                ...
                session.createStatement().use { statement ->
                    statement.execute(
                        "CREATE TABLE IF NOT EXISTS identity_deletion_signature_received (" +
                                " owned_identity BLOB NOT NULL, " +
                                " signature BLOB NOT NULL, " +
        ...
        fun exists(
            ...
            protocolManagerSession.session.prepareStatement(
                "IdentityDeletionSignatureReceived.exists",
