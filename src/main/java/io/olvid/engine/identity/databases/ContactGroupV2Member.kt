...
class ContactGroupV2Member : ObvDatabase {
    ...
    fun setPermissions(permissionStrings: MutableList<String>) {
        ...
        identityManagerSession.session.prepareStatement(
            "ContactGroupV2Member.setPermissions",
            "UPDATE " + TABLE_NAME +
    ...
    fun setGroupInvitationNonce(groupInvitationNonce: ByteArray?) {
        identityManagerSession.session.prepareStatement(
            "ContactGroupV2Member.setGroupInvitationNonce",
            "UPDATE " + TABLE_NAME +
    ...
    companion object {
        const val TABLE_NAME: String = "contact_group_v2_member"
        ...
        fun create(
            ...
            try {
                val contactGroupMember = ContactGroupV2Member(
        ...
        fun get(
            ...
            identityManagerSession.session.prepareStatement(
                ...
                statement.executeQuery().use { res ->
                    if (res.next()) {
                        return ContactGroupV2Member(identityManagerSession, res)
                    } else {
                        return null
                    }
                }
        ...
        fun getAll(
            ...
            ownedIdentity: Identity?,
            groupIdentifier: GroupV2.Identifier?
        ): MutableList<ContactGroupV2Member?>? {
            if ((ownedIdentity == null) || (groupIdentifier == null)) {
                return null
            }
            identityManagerSession.session.prepareStatement(
                "ContactGroupV2Member.getAll",
                "SELECT * FROM " + TABLE_NAME +
                        " WHERE " + GROUP_UID + " = ? " +
                        " AND " + SERVER_URL + " = ? " +
                        " AND " + CATEGORY + " = ? " +
                ...
                statement.setBytes(4, ownedIdentity.getBytes())
                statement.executeQuery().use { res ->
                    val list: MutableList<ContactGroupV2Member?> =
                        ArrayList<ContactGroupV2Member?>()
                    while (res.next()) {
                        list.add(ContactGroupV2Member(identityManagerSession, res))
                    }
                    return list
                }
            }
        ...
        fun isContactMemberOfAGroupV2(
            ...
            identityManagerSession.session.prepareStatement(
                "ContactGroupV2Member.isContactMemberOfAGroupV2",
        ...
        @Throws(SQLException::class)
        fun getKeycloakGroupV2IdentifiersWhereContactIsMember(
            identityManagerSession: IdentityManagerSession,
            ownedIdentity: Identity?,
            contactIdentity: Identity?
        ...
        fun upgradeTable(session: Session, oldVersion: Int, newVersion: Int) {
            ...
            if (oldVersion < 32 && newVersion >= 32) {
                session.createStatement().use { statement ->
                    ...
                    statement.execute(
                        "CREATE TABLE contact_group_v2_member (" +
                                "group_uid BLOB NOT NULL, " +
                                "server_url TEXT NOT NULL, " +
                                "category INT NOT NULL, " +
                                "owned_identity BLOB NOT NULL, " +
        ...
        fun backupAll(
            ...
            val members: MutableList<ContactGroupV2Member?> =
