...
class EncryptionEciesMDCPublicKey(key: HashMap<DictionaryKey, Encoded>) :
    EncryptionEciesPublicKey(ALGO_IMPL_KEM_ECIES_MDC_AND_DEM_CTR_AES256_THEN_HMAC_SHA256, key) {
    companion object {
        val COMPACT_KEY_LENGTH: Int = 1 + Suite.getCurve(EdwardCurve.MDC).byteLength

        @JvmStatic
        @Throws(DecodingException::class)
        fun of(compactKeyBytes: ByteArray): EncryptionEciesMDCPublicKey {
            if ((compactKeyBytes[0] != ALGO_IMPL_KEM_ECIES_MDC_AND_DEM_CTR_AES256_THEN_HMAC_SHA256) || (compactKeyBytes.size != COMPACT_KEY_LENGTH)) {
            ...
            return EncryptionEciesMDCPublicKey(key)
        }
    }
}
